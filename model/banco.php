...
class Banco{
    ...
    public function getAgendamentos($id){
        try{
            if(isset($id) && $id > 0){
                $stmt = $this->mysqli->query("SELECT * FROM cadastrar WHERE id = '" . $id . "';");
            }else{
                $stmt = $this->mysqli->query("SELECT * FROM cadastrar;");
            }
    ...
    public function updateAgendamentos($id, $email,$senha,$endereco,$bairro,$cep, $cidade, $estado){
        $stmt = $this->mysqli->query("UPDATE `cadastrar` SET `email` = '" . $email . "', `senha` =  '" . $senha . "', `endereco` =  '" . $endereco . "', `bairro` =  '" . $bairro . "', `cep` =   '" . $cep . "', `cidade` = '" . $cidade . "', `estado` = '" . $estado . "' WHERE `id` =  '" . $id . "';");
         if( $stmt > 0){
             return true ;
         }else{
             return false;
     ...
     public function deleteAgendamentos($id){
        $stmt = $this->mysqli->query("DELETE FROM cadastrar WHERE `id` =  '" . $id . "';");
        if( $stmt > 0){
            return true ;
        }else{
            return false;
